...
// magic.cpp - Magic Bitboards Implementation
#include "magic.h"
#include <array>
#include <cstring>
...
void init() {
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        totalR += (1 << popcount64(rm));
        
        U64 bm = bishopMaskFor(sq);
        bishopMasks[sq] = bm;
        bishopShifts[sq] = 64 - popcount64(bm);
        totalB += (1 << popcount64(bm));
    }
    
    rookTable.assign(totalR, 0);
    bishopTable.assign(totalB, 0);
    int roff = 0, boff = 0;
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        // Carry-rippler to iterate all subsets
        U64 occ = 0;
        for (int i = 0; ; ) {
