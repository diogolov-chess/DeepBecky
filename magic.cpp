...
static U64 rayAtt(int sq, int dx, int dy, U64 occ){
    ...
    U64 a=0; x+=dx; y+=dy;
    while(onB(x,y)){ int t=y*8+x; a|=setMask(t); if(occ & setMask(t)) break; x+=dx; y+=dy; }
    return a;
}
static U64 rookAttGivenOcc(int sq, U64 occ){
    U64 a=0;
    a |= rayAtt(sq, 1,0,occ); a |= rayAtt(sq,-1,0,occ);
    a |= rayAtt(sq, 0,1,occ); a |= rayAtt(sq, 0,-1,occ);
    return a;
}
static U64 bishopAttGivenOcc(int sq, U64 occ){
    U64 a=0;
    a |= rayAtt(sq, 1,1,occ); a |= rayAtt(sq,-1,1,occ);
    a |= rayAtt(sq, 1,-1,occ); a |= rayAtt(sq,-1,-1,occ);
    return a;
}

void init(){
    // First pass: compute masks, shifts, offsets and total sizes
    int totalR=0, totalB=0;
    for(int sq=0;sq<64;sq++){ 
        U64 rm=rookMaskFor(sq); rookMasks[sq]=rm; rookShifts[sq]=64 - popcount64(rm); totalR += (1 << popcount64(rm)); 
        U64 bm=bishopMaskFor(sq); bishopMasks[sq]=bm; bishopShifts[sq]=64 - popcount64(bm); totalB += (1 << popcount64(bm));
    }
    rookTable.assign(totalR, 0); bishopTable.assign(totalB, 0);
    int roff=0, boff=0;

    for(int sq=0;sq<64;sq++){ 
        U64 mask = rookMasks[sq]; rookOffsets[sq]=roff;
        ...
        for(int i=0;;){
            ...
            rookTable[roff + (int)idx] = rookAttGivenOcc(sq, occ);
            i++; occ = (occ - mask) & mask; if(!occ) break;
        }
        roff += subsetCount;
    }
    for(int sq=0;sq<64;sq++){ 
        U64 mask = bishopMasks[sq]; bishopOffsets[sq]=boff;
        int rbits = popcount64(mask);
        int subsetCount = 1 << rbits;
        U64 occ=0;
        for(int i=0;;){
            U64 idx = ((occ & mask) * bishopMagics[sq]) >> bishopShifts[sq];
            bishopTable[boff + (int)idx] = bishopAttGivenOcc(sq, occ);
            i++; occ = (occ - mask) & mask; if(!occ) break;
        }
        boff += subsetCount;
    }
