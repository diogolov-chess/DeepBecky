...
int Position::generatePseudo(Move* mv, bool capturesOnly) {
    ...
    // === Knights (no switch, dedicated loop) ===
    {
        U64 bb = bitboards[us == WHITE ? WKNIGHT : BKNIGHT];
        while (bb) {
            int from_sq = pop_lsb(&bb);
            U64 attacks = KNIGHT_ATK_BB[from_sq];
            U64 caps = attacks & opp_pieces;
            while (caps) {
                int to_sq = pop_lsb(&caps);
                addMove(mv, count, from_sq, to_sq, MOVE_FLAG_CAPTURE);
            }
            if (!capturesOnly) {
                U64 quiets = attacks & empty_squares;
                while (quiets) {
                    int to_sq = pop_lsb(&quiets);
                    addMove(mv, count, from_sq, to_sq, MOVE_TYPE_QUIET);
                }
            }
        }
    }
    ...
    // === Rooks ===
    {
        ...
        while (bb) {
            int from_sq = pop_lsb(&bb);
            U64 attacks = Magic::rookAttacks(from_sq, all_pieces);
            U64 caps = attacks & opp_pieces;
            while (caps) {
                int to_sq = pop_lsb(&caps);
                addMove(mv, count, from_sq, to_sq, MOVE_FLAG_CAPTURE);
    ...
    // === King (direct square, no bitboard iteration needed) ===
    {
        int from_sq = king_sq[us];
        U64 attacks = KING_ATK_BB[from_sq];
        U64 caps = attacks & opp_pieces;
        while (caps) {
            int to_sq = pop_lsb(&caps);
            addMove(mv, count, from_sq, to_sq, MOVE_FLAG_CAPTURE);
    ...
    if (us == WHITE) {
        ...
        while (nl) {
            int to = pop_lsb(&nl);
            addMove(mv, count, to - 7, to, MOVE_FLAG_CAPTURE);
        }
        U64 nr = capR & ~Rank8BB;
        while (nr) {
            int to = pop_lsb(&nr);
            addMove(mv, count, to - 9, to, MOVE_FLAG_CAPTURE);
        }
        ...
        while (pl) {
            int to = pop_lsb(&pl);
            for (int p : promoPieces)
                addMove(mv, count, to - 7, to, makePromoCaptureMoveType(p));
        }
        U64 pr = capR & Rank8BB;
        while (pr) {
            int to = pop_lsb(&pr);
            for (int p : promoPieces)
                addMove(mv, count, to - 9, to, makePromoCaptureMoveType(p));
        }
        ...
        if (ep_file > 0) {
            ...
            while (f) {
                int from = pop_lsb(&f);
                addMove(mv, count, from, ep_sq, MOVE_TYPE_EP);
            }
        }
    } else {
        {
