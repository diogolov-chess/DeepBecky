    ...
    for (int s1 = 0; s1 < 64; ++s1) {
        for (int s2 = 0; s2 < 64; ++s2) {
            ...
            if (dx == 0 && dy != 0) {  // Vertical
                ...
                sameLine = true;
                stepX = (dx > 0) ? 1 : -1;
            } else if (std::abs(dx) == std::abs(dy)) {  // Diagonal
                sameLine = true;
                stepX = (dx > 0) ? 1 : -1;
                stepY = (dy > 0) ? 1 : -1;
            }
            ...
            if (sameLine) {
                // BETWEEN_BB: squares between s1 and s2 (exclusive)
                U64 between = 0;
                int cx = x1 + stepX;
                int cy = y1 + stepY;
                while (cx != x2 || cy != y2) {
                ...
                // LINE_BB: complete line through s1 and s2
                int dir = -1;
                for (int d = 0; d < 8; ++d) {
